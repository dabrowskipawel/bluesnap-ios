...
public class BSApiManager: NSObject {
    ...
    internal static var apiGenerateTokenFunc:
        (_ completion: @escaping (BSToken?, BSErrors?) -> Void) -> Void = { completion in
    ...
    */
     ...
     */
    open class func setGenerateBsTokenFunc(
     ...
     */
    static func getSdkData(
        baseCurrency: String?, completion: @escaping (BSSdkConfiguration?, BSErrors?) -> Void
    ) {

        let bsToken = getBsToken()

        NSLog("BlueSnap; getSdkData")
        BSApiCaller.getSdkData(
            bsToken: bsToken, baseCurrency: baseCurrency,
            completion: {
                ...
                if resultError == .unAuthorised {
                    ...
                    regenerateToken(executeAfter: {
                        BSApiCaller.getSdkData(
                            bsToken: getBsToken(), baseCurrency: baseCurrency,
                            completion: { sdkData2, resultError2 in
     ...
     */
    public static func submitPurchaseDetails(
        purchaseDetails: BSExistingCcSdkResult,
        completion: @escaping (BSCreditCard, BSErrors?) -> Void
    ) {

        let cc = purchaseDetails.creditCard
        BSApiManager.submitPurchaseDetails(
            ccNumber: nil, expDate: cc.getExpirationForSubmit(), cvv: nil,
            last4Digits: cc.last4Digits, cardType: cc.ccType,
            billingDetails: purchaseDetails.billingDetails,
            shippingDetails: purchaseDetails.shippingDetails, storeCard: true,
            fraudSessionId: BlueSnapSDK.fraudSessionId, completion: completion)
    }
     ...
     */
    public static func submitPurchaseDetails(
        ccNumber: String?, expDate: String?, cvv: String?, last4Digits: String?, cardType: String?,
        billingDetails: BSBillingAddressDetails?, shippingDetails: BSShippingAddressDetails?,
        storeCard: Bool?, fraudSessionId: String?,
        completion: @escaping (BSCreditCard, BSErrors?) -> Void
    ) {

        let tokenizeRequest = BSTokenizeRequest()
        if let ccNumber = ccNumber {
            tokenizeRequest.paymentDetails = BSTokenizeNewCCDetails(
                ccNumber: ccNumber, cvv: cvv, ccType: cardType, expDate: expDate)
        } else {
            tokenizeRequest.paymentDetails = BSTokenizeExistingCCDetails(
                lastFourDigits: last4Digits, ccType: cardType, expDate: expDate)
        }
    ...
    static func submitCcn(ccNumber: String, completion: @escaping (BSCreditCard, BSErrors?) -> Void)
    {
        ...
        submitPurchaseDetails(
            ccNumber: ccNumber, expDate: nil, cvv: nil, last4Digits: nil, cardType: nil,
            billingDetails: nil, shippingDetails: nil, storeCard: nil, fraudSessionId: nil,
    ...
    static func getSupportedPaymentMethods(completion: @escaping ([String]?, BSErrors?) -> Void) {
        ...
        BSApiCaller.getSupportedPaymentMethods(
            bsToken: bsToken,
            completion: {
                ...
                if resultError == .unAuthorised {
                    BSApiCaller.isTokenExpired(
                        bsToken: bsToken,
                        completion: { isExpired in
                            if isExpired {
                                // regenerate Token and try again
                                regenerateToken(executeAfter: {
                                    ...
                                    BSApiCaller.getSupportedPaymentMethods(
                                        bsToken: getBsToken(),
                                        completion: {
                                            resultSupportedPaymentMethods2, resultError2 in

                                            NSLog(
                                                "BlueSnap; getSupportedPaymentMethods retry completion"
                                            )
                                            if resultError2 == nil {
                                                supportedPaymentMethods =
                                                    resultSupportedPaymentMethods2
                                                self.lastSupportedPaymentMethodsFetchDate = Date()
                                            }
     ...
     */
    static func isSupportedPaymentMethod(
        paymentType: BSPaymentType, supportedPaymentMethods: [String]?
    ) -> Bool {
    ...
    static func isNewCCOnlyPaymentMethod() -> Bool {
        ...
        let isPayPalSupported = isSupportedPaymentMethod(
            paymentType: BSPaymentType.PayPal,
            supportedPaymentMethods: BSApiManager.supportedPaymentMethods)
        let isApplePaySupported = BlueSnapSDK.applePaySupported(
            supportedPaymentMethods: BSApiManager.supportedPaymentMethods,
            supportedNetworks: BlueSnapSDK.applePaySupportedNetworks
        ).canMakePayments

        let existingCreditCards =
            shopper == nil ? false : (shopper?.existingCreditCards.count == 0 ? false : true)
     ...
     */
    static func createPayPalToken(
        purchaseDetails: BSPayPalSdkResult, withShipping: Bool,
        completion: @escaping (String?, BSErrors?) -> Void
    ) {
        ...
        DispatchQueue.global().async {
            ...
            BSApiCaller.createPayPalToken(
                bsToken: bsToken, purchaseDetails: purchaseDetails, withShipping: withShipping,
                completion: {
                    ...
                    if resultError == .paypalUTokenAlreadyUsed {
                        ...
                        regenerateToken(executeAfter: {
                            BSApiCaller.createPayPalToken(
                                bsToken: getBsToken(), purchaseDetails: purchaseDetails,
                                withShipping: withShipping,
                                completion: { resultToken2, resultError2 in
    ...
    /**
     Submit data to be submitted to BLS server under the current token, to be used later for server-to-server actions
     */
    open class func requestAuthWith3ds(
        currency: String, amount: String, cardinalToken: String,
        completion: @escaping (BS3DSAuthResponse?, BSErrors?) -> Void
    ) {

        let authRequest = BS3DSAuthRequest(
            currencyCode: currency, amount: amount, jwt: cardinalToken)

        BSApiCaller.requestAuthWith3ds(
            bsToken: getBsToken(), authRequest: authRequest, completion: completion)
    }

    /**
     Process 3DS result to BLS server under the current token
     */
    open class func processCardinalResult(
        cardinalToken: String, resultJwt: String,
        completion: @escaping (BS3DSProcessResultResponse?, BSErrors?) -> Void
    ) {

        let processResultRequest = BS3DSProcessResultRequest(
            jwt: cardinalToken, resultJwt: resultJwt)

        BSApiCaller.processCardinalResult(
            bsToken: getBsToken(), processResultRequest: processResultRequest,
            completion: completion)
    }
    ...
    open class func updateShopper(completion: @escaping ([String: String], BSErrors?) -> Void) {
        ...
        {
            requestBody["paymentSources"] = [
                "creditCardInfo": [["pfToken": getBsToken()!.tokenStr]]
            ]
        }
        ...
        let parseFunction: (Int, Data?) -> ([String: String], BSErrors?) = BSApiCaller
            .parseGenericResponse

        BSApiCaller.updateShopper(
            bsToken: getBsToken(), requestBody: requestBody, parseFunction: parseFunction,
            completion: { resultData, error in
                ...
                if error == BSErrors.expiredToken || error == BSErrors.tokenNotFound {
                    ...
                    regenerateToken(executeAfter: {
                        BSApiCaller.updateShopper(
                            bsToken: getBsToken(), requestBody: requestBody,
                            parseFunction: parseFunction, completion: checkErrorAndComplete)
                    })
                } else {
                    checkErrorAndComplete(resultData, error)
                }
    ...
    private static func submitCcDetails(
        tokenizeRequest: BSTokenizeRequest, completion: @escaping (BSCreditCard, BSErrors?) -> Void
    ) {
        ...
        submitTokenizedDetails(
            ...
            completion: { resultData, error in
                NSLog("BlueSnap; submitCcDetails completion")
                fillCcDetailsAndComplete(
                    tokenizeRequest: tokenizeRequest, resultData: resultData, error: error,
                    completion: completion)
            })
    }

    internal static func fillCcDetailsAndComplete(
        tokenizeRequest: BSTokenizeRequest, resultData: [String: String], error: BSErrors?,
        completion: @escaping (BSCreditCard, BSErrors?) -> Void
    ) {
