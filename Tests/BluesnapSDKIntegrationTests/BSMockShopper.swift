...
class MockShopper {
    ...
    init(
        creditCardInfo: [([String: String], [String: String])], email: String? = nil,
        shippingContactInfo: [String: String]? = nil, fullBillingRequired: Bool,
        emailRequired: Bool, shippingRequired: Bool
    ) {

        for item in creditCardInfo {
            self.creditCardInfo[item.1["cardLastFourDigits"]!] = MockShopperCreditCardInfo(
                billingContactInfo: item.0, creditCard: item.1)
        }

        self.shippingContactInfo = shippingContactInfo
        self.email = email

        shopperCheckoutRequirements = MockShopperCheckoutRequirements(
            fullBillingRequired: fullBillingRequired, emailRequired: emailRequired,
